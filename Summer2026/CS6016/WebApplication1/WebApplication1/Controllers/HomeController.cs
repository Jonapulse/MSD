...
public class HomeController(MyDbContext dbContext) : Controller
{
     // WARNING:
    ...
    {
        bool loginSuccessful = dbContext.Patrons
    ...
    public ActionResult AllTitles()
    {
        var allTitles = dbContext.Titles
            .SelectMany(
                title => title.Inventories.DefaultIfEmpty(),
                (title, inventory) => new
                {
                    ISBN = title.Isbn,
                    Title = title.Title1,
                    Author = title.Author,
                    Serial = inventory != null ? inventory.Serial : -1,
                    Name = inventory != null && inventory.Checkedout != null && inventory.Checkedout.CardnumNavigation != null ? 
                        inventory.Checkedout.CardnumNavigation.Name : ""
                });
        return Json(allTitles);
    ...
    {
        var userCheckedOut = dbContext.Patrons
            .Where(p => p.Name == user)
            .SelectMany(
                patron => patron.Checkedouts,
                (patron, checkedOut) => new
                {
                    Title = checkedOut.SerialNavigation.IsbnNavigation.Title1,
                    Author = checkedOut.SerialNavigation.IsbnNavigation.Author,
                    Serial = checkedOut.Serial
                });
        return Json(userCheckedOut);
    }
    ...
    {
        Checkedout checkMeOut = new Checkedout()
        {
            Serial = serial,
            Cardnum = card
        };
        dbContext.Checkedouts.Add(checkMeOut);
    ...
    {
        var toReturn = dbContext.Checkedouts
            .Where(c => c.Serial == serial);
        dbContext.Checkedouts.RemoveRange(toReturn);
        dbContext.SaveChanges();
        return Json(new { success = true });
    }
