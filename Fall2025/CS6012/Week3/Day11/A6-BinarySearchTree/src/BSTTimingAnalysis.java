...
public class BSTTimingAnalysis
{
    public static void main(String[] args) {
        ...
        int[] problemSizes = {(int)Math.pow(2,10), (int)Math.pow(2,11),(int)Math.pow(2,12),(int)Math.pow(2,13), (int)Math.pow(2,14)};/*(int)Math.pow(2,15), (int)Math.pow(2,16), (int)Math.pow(2,17), (int)Math.pow(2,18),
        ...
        TreeBuildingTest t = new TreeBuildingTest(problemSizes, 10/*0*/);
        var results = t.run();

        System.out.println("n, time");
        for(var result: results){
            System.out.println(result.n() + ", " + (result.avgNanoSecs() / 1_000_000_000));
        }
    }
}
