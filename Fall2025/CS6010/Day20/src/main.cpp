...
int main(int argc, const char * argv[]) {
    ...
    const sf::Font font("BitcountGridDouble.ttf");
    sf::Text text(font, "Hello SFML", 50);
    sf::Texture characterImage = sf::Texture("alien.png");
    ...
    while(window.isOpen()){
        ...
        sprite.move(sf::Vector2f(timeElapsed.asMilliseconds(), 0)); // +x go right, +y go down.
        window.draw(text);
        window.draw(sprite);
        window.display();
    }
