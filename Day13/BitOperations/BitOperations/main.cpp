...
uint32_t SetByte( uint32_t input, uint8_t value, int b )
{
    ...
    for(int i = 0; i < 4; i++)
    {
        ...
        if(i == b)
            ...
            bytePiece = GetByte(input, i) << i * 8;
        returnBytes = (returnBytes | bytePiece);
...
int Increment( uint32_t x ){
    ...
    while(firstZeroIndex < 32 && GetBit(x, firstZeroIndex))
        firstZeroIndex++;
    uint32_t incMask = 0xffffffff;
    //If a 0 was found, create a mask of 0s and 1s, with 1s starting where the 0 was found
    if(firstZeroIndex < 32)
        incMask >>= (32 - firstZeroIndex - 1);
    return x ^ incMask;
}
