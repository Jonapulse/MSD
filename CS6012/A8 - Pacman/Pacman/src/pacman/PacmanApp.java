...
public class PacmanApp {
    ...
    private static void createAndShowGUI(char[][] maze) {
        ...
        int desiredWidth = maze[0].length*BlockSize;
        int desiredHeight = maze.length*BlockSize;
        ...
        if(desiredHeight > maxHeight){
            scale = Math.min(scale, maxHeight/desiredHeight);
        }
        BlockSize = (int)(BlockSize*scale);
        CornerSize = (int)(CornerSize*scale);
        DotRadius = (int)(DotRadius*scale);
    ...
    private static class GamePanel extends JPanel{
        ...
        public void paintComponent(Graphics g) {
            ...
            for(var row = 0; row < maze.length; row++){
                for(var col = 0; col < maze[0].length; col++){
                    ...
                    if(maze[row][col] == 'X'){
                        ...
                        int centerX = (int)((col + 0.5)*BlockSize);
                        int centerY = (int)((row + 0.5)*BlockSize);
                        int cherrySize = (int)(20*scale);
                        g2.fillOval(centerX - cherrySize, centerY, cherrySize, cherrySize);
                        g2.fillOval(centerX - cherrySize/2, centerY, cherrySize, cherrySize);
                        g2.setColor(Color.YELLOW);
                        g2.setStroke(new BasicStroke(4));
                        g2.drawLine(centerX, centerY, centerX + cherrySize/3, centerY - cherrySize/3);
            ...
            final int pacmanRadius = (int)(20*scale);
